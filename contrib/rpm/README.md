...
The RPM spec file provided here is for Avian-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://avn.network/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Avian git repository](https://github.com/AvianNetwork/Avian) but are not part of
...
Currently only `Source10` is used, for the example `avian.conf` file.
...
in the avian git repository. At present only an SVG version of the Avian
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the avian
...
needed when 0.12.1 is released, a proper fix is already in the Avian git
master branch.
...
idea to build Avian Core against a static version of that library compiled
...
The `avian.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `avian-server` package. At this point, most RPM based
...
The `avian-server` package should have SELinux support. How to properly do
...
Avian-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/avian/). He has been
packaging Avian for Fedora at least since 2012.
...
4. I renamed the `avian` package that contains the Qt GUI to `avian-core` as
that appears to be how the general population refers to it, in contrast to
`avian-xt` or `avian-classic`. I wanted to make sure the general population
