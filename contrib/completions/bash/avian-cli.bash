# bash programmable completion for avian-cli(1)
# Copyright (c) 2012-2022 The Avian Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $avian-cli for RPC
_avian_rpc() {
...
_avian_cli() {
    ...
    # save and use original argument to invoke avian-cli for -help, help and RPC
    # as avian-cli might not be in $PATH
    avian_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($avian_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_avian_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _avian_cli avian-cli
