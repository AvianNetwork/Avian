...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing avian-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='avian-cli', help='avian-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Avian address to which the faucet should send')
...
parser.add_argument('avian_cli_args', nargs='*', help='Arguments to pass on to avian-cli (default: -signet)')

args = parser.parse_args()

if args.avian_cli_args == []:
    args.avian_cli_args = ['-signet']


def avian_cli(rpc_command_and_params):
    argv = [args.cmd] + args.avian_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = avian_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = avian_cli(['getnewaddress', 'faucet', 'bech32'])
