# Bootstrappable Avian Core Builds

This directory contains the files necessary to perform bootstrappable Avian
Core builds.
...
To build Avian Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Avian Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/avian-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/AvianNetwork/guix.sigs).
