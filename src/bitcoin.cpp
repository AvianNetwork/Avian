...
Options:
  -m, --multiprocess     Run multiprocess binaries avian-node, avian-gui.
  -M, --monolithic       Run monolithic binaries aviand, avian-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'avian-qt [ARGS]' or 'avian-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'aviand [ARGS]' or 'avian-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'avian-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'avian-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'avian-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_avian [ARGS]'.
  chainstate [ARGS] Run avian kernel chainstate util, equivalent to running 'avian-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_avian [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_avian-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "avian-gui" : "avian-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "avian-node" : "aviand");
        } else if (cmd.command == "rpc") {
            args.emplace_back("avian-cli");
            // Since "avian rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("avian-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("avian-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_avian");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("avian-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_avian");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_avian-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("avian-util");
        } else {
...
//! Execute the specified aviand, avian-qt or other command line in `args`
...
//!             be a relative path to a aviand, avian-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/avian ...
