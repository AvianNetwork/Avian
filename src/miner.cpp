...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, const POW_TYPE powType)
{
    ...
    if (!IsDualAlgoEnabled(pindexPrev, chainparams.GetConsensus()) && powType != 0)
        throw std::runtime_error("Error: Won't attempt to create a non-x16r block before Dual Algo activation");

    // Dual Algo: If Minotaurx Algo is enabled, encode desired pow type.
...
void static AvianMiner(const CChainParams& chainparams, const POW_TYPE powType)
{
    ...
    try {
        ...
        while (true) {
            ...
            // Build block with Minotaurx algo powType
