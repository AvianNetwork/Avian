...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " avian-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The avian-tx tool is used for creating and modifying transactions.\n\n"
                "avian-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded transaction, or with \"-create [commands]\" to create a hex-encoded transaction.\n"
                "\n"
                "Usage: avian-tx [options] <hex-tx> [commands]\n"
                "or:    avian-tx [options] -create [commands]\n"
                "\n";
