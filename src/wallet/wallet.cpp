...

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if (fGetAVN) {        // Looking for AVN Tx OutPoints Only
                    if (fAVNLimitHit) // We hit our limit
                        continue;

                    // We only want AVN OutPoints. Don't include Asset OutPoints
                    if (isAssetScript)
                        continue;
...
bool CWallet::CreateTransactionAll(const std::vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool fNewAsset, const std::vector<CNewAsset> assets, const CTxDestination destination, bool fTransferAsset, bool fReissueAsset, const CReissueAsset& reissueAsset, const AssetType& assetType, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-avian-address
            ...
            while (true) {
                ...
                for (const auto& recipient : vecSend) {
                    ...
                    /** AVN START */
                    // Check to see if you need to make an asset data outpoint OP_AVN_ASSET data
                    ...
                    if (IsDust(txout, ::dustRelayFee) && !IsScriptTransferAsset(recipient.scriptPubKey)) /** AVN START */ /** AVN END */
                    {
