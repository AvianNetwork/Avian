...
bool CWallet::SignTransaction(CMutableTransaction& tx) const
{
    ...
    // Avian: Always use SIGHASH_FORKID for replay protection (UAHF permanently active)
    return SignTransaction(tx, coins, SIGHASH_ALL | SIGHASH_FORKID, input_errors);
}
...
static util::Result<fs::path> GetWalletPath(const std::string& name)
{
    ...
    // 5. An absolute path to a data file (for legacy wallets in the data dir root).
    ...
    if (!(path_type == fs::file_type::not_found || path_type == fs::file_type::directory ||
          ...
          (path_type == fs::file_type::regular && (fs::PathFromString(name).filename() == fs::PathFromString(name) ||
                                                   fs::PathFromString(name).is_absolute())))) {
    ...
    if (!gArgs.GetBoolArg("-walletcrosschain", DEFAULT_WALLETCROSSCHAIN)) {
        ...
        if (batch.ReadBestBlock(locator) && locator.vHave.size() > 0 && chain.getHeight()) {
            ...
            if (chain.getBlockHash(0) != locator.vHave.back()) {
                error = Untranslated("Wallet files should not be reused across chains. Restart aviand with -walletcrosschain to override.");
                return false;
            }
        }
    }
