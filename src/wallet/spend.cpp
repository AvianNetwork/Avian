                                     ...
                                     const CoinFilterParams& params)
{
    ...
    for (const auto& [outpoint, txo] : wallet.GetTXOs()) {
        ...
        // Only interested in asset scripts
        ...
        // Extract asset name from script
                  ...
                  std::map<std::string, CAmount>& nValueRet)
{
    ...
    for (const auto& [assetName, nTargetValue] : mapAssetTargets) {
        ...
        if (it == mapAssetCoins.end())
            return false; // No coins available for this asset

        const std::vector<COutput>& vCoins = it->second;

        // Simple greedy selection: sort by value descending, pick until target met
        ...
        if (nValueSelected < nTargetValue)
            return false; // Insufficient asset funds
    }
