...
UniValue importprivkey(const JSONRPCRequest& request)
{
    ...
    std::string strSecret = params.GetString(0);
    std::string strLabel = params.GetString(1, "");
    bool fRescan = params.GetBool(2, true);
...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    bool fP2SH = params.GetBool(3, false);
    ...
    } else if (IsHex(params.GetString(0))) {
        std::vector<unsigned char> data(ParseHex(params.GetString(0)));
...
UniValue importprunedfunds(const JSONRPCRequest& request)
{
    ...
    if (!DecodeHexTx(tx, params.GetString(0)))
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "TX decode failed");
    uint256 hashTx = tx.GetHash();
    CWalletTx wtx(pwallet, MakeTransactionRef(std::move(tx)));

    CDataStream ssMB(ParseHexV(params.GetString(1), "proof"), SER_NETWORK, PROTOCOL_VERSION);
...
UniValue removeprunedfunds(const JSONRPCRequest& request)
{
    ...
    RPC::Params params(request);
    uint256 hash;
    hash.SetHex(params.GetString(0));
...
UniValue importwallet(const JSONRPCRequest& request)
{
    ...
    RPC::Params params(request);
    std::ifstream file;
    file.open(params.GetString(0).c_str(), std::ios::in | std::ios::ate);
    if (!file.is_open())
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    RPC::Params params(request);
    boost::filesystem::path filepath = params.GetString(0);
...
UniValue importmulti(const JSONRPCRequest& mainRequest)
{
    ...
    RPC::Params params(mainRequest);
    const UniValue& requests = params.GetArray(0);
