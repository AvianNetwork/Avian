...
UniValue getmywords(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("getmywords", "") + HelpExampleRpc("getmywords", ""));
...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new Avian address for receiving payments.\n"
            "If 'account' is specified (DEPRECATED), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) DEPRECATED. The account name for the address to be linked to. If not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"address\"    (string) The new avian address\n"
            "\nExamples:\n" +
...
UniValue getaccountaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "\"address\"          (string) The account avian address\n"
            "\nExamples:\n" +
...
UniValue setaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The avian address to be associated with an account.\n"
...
UniValue getaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The avian address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"address\"         (string) a avian address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n" +
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 7)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"            (string, required) The avian address to send to.\n"
            ...
            CURRENCY_UNIT + " to send. eg 0.1\n"
                            ...
                            "                             The recipient will receive less avians than you enter in the amount field.\n"
...
UniValue sendfromaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            HelpRequiringPassphrase(pwallet) +
            "\nArguments:\n"
            "1. \"from_address\"       (string, required) The avian address to send from.\n"
            "2. \"to_address\"            (string, required) The avian address to send to.\n"
            ...
            CURRENCY_UNIT + " to send. eg 0.1\n"
                            ...
                            "\"txid\"                  (string) The transaction id.\n"
                            "\nExamples:\n" +
            HelpExampleCli("sendfromaddress", "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" \"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1") + HelpExampleCli("sendfromaddress", "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" \"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 \"donation\" \"seans outpost\"") + HelpExampleCli("sendfromaddress", "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" \"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 \"\" \"\" true") + HelpExampleRpc("sendfromaddress", "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" \"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\", 0.1, \"donation\", \"seans outpost\""));
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The avian address\n"
...
UniValue signmessage(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            HelpRequiringPassphrase(pwallet) + "\n"
                                               "\nArguments:\n"
                                               "1. \"address\"         (string, required) The avian address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The avian address for transactions.\n"
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 6)
        throw std::runtime_error(
            ...
            HelpRequiringPassphrase(pwallet) + "\n"
                                               ...
                                               "                       the spend.\n"
                                               "2. \"toaddress\"         (string, required) The avian address to send funds to.\n"
                                               "3. amount                (numeric or string, required) The amount in " +
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            HelpRequiringPassphrase(pwallet) + "\n"
                                               ...
                                               "    {\n"
                                               "      \"address\":amount   (numeric or string) The avian address is the key, the numeric amount (can be string) in " +
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3) {
        std::string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
                          ...
                          "2. \"keys\"         (string, required) A json array of avian addresses or hex-encoded public keys\n"
                          "     [\n"
                          "       \"address\"  (string) avian address or hex-encoded public key\n"
                          "       ...,\n"
                          "     ]\n"
                          "3. \"account\"      (string, optional) DEPRECATED. An account to assign the addresses to.\n"

                          "\nResult:\n"
                          "\"address\"         (string) A avian address associated with the keys.\n"
    ...
    bool operator()(const CKeyID& keyID)
    {
        if (pwallet) {
            ...
            if (!ProduceSignature(DummySignatureCreator(pwallet), witscript, sigs) ||
                !VerifyScript(sigs.scriptSig, witscript, &sigs.scriptWitness, | SCRIPT_VERIFY_WITNESS_PUBKEYTYPE, DummySignatureCreator(pwallet).Checker())) {
                return false;
            }
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The avian address of the transaction. Not present for \n"
    ...
    for (CWallet::TxItems::const_iterator it = txOrdered.begin(); it != txOrdered.end(); ++it) {
        ...
        if (wtx->GetDepthInMainChain() <= 0 && !wtx->InMempool() && wtx->IsFromMe(filter)) {
            if (!verbose) {
                ...
                ListTransactions(pwallet, *wtx, "*", 0, true, details, filter);
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The avian address of the transaction. Not present for move transactions (category = move).\n"
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            CURRENCY_UNIT + ". This is negative and only available for the \n"
                            ...
                            "    {\n"
                            "      \"account\" : \"accountname\",      (string) DEPRECATED. The account name involved in the transaction, can be \"\" for the default account.\n"
                            "      \"address\" : \"address\",          (string) The avian address involved in the transaction\n"
                            "      \"category\" : \"send|receive\",    (string) The category, either 'send' or 'receive'\n"
                            "      \"amount\" : x.xxx,                 (numeric) The amount in " +
            CURRENCY_UNIT + "\n"
                            "      \"label\" : \"label\",              (string) A comment for the address/transaction, if any\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (!pwallet->IsCrypted() && (request.fHelp || request.params.size() != 1)) {
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending avian\n" + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
    ...
    StartShutdown();
    return "wallet encrypted; Avian server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "3. \"addresses\"      (string) A json array of avian addresses to filter\n"
            "    [\n"
            ...
            CURRENCY_UNIT + "\n"
                            ...
                            "    \"vout\" : n,               (numeric) the vout value\n"
                            "    \"address\" : \"address\",    (string) the avian address\n"
    ...
    if (!request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Avian address: ") + input.get_str());
            }
    ...
    if (!request.params[1].isNull()) {
        if (request.params[1].type() == UniValue::VBOOL) {
            ...
            if (options.exists("changeAddress")) {
                ...
                if (!IsValidDestination(dest)) {
                    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid avian address");
                }
...
{
    throw std::runtime_error("bumpfee has been deprecated on the AVN Wallet.");
...
UniValue generate(const JSONRPCRequest& request)
{
    ...
    return generateBlocks(coinbase_script, num_generate, max_tries, true, powType);
}
...
// Local version of strPad used by ConsolidateUtxosListCoins
...
// Function used by consolidateutxos below to refresh coin list
void ConsolidateUtxosListCoins(std::map<string, vector<COutput>>& mapCoins, CWallet* const pwallet)
{
...
UniValue consolidateutxos(const JSONRPCRequest& request)
{
    // Get wallet
    CWallet* const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
    ...
    if (request.fHelp || request.params.size() < 1)
        throw std::runtime_error("consolidateutxos <address> [min_utxos=2000] [max_batches=0] [min_amount=0.01] [max_amount=25]\n"
                                 ...
                                 "4. min_amount     (numeric, optional, default=0.01) Minimum UTXO amount in AVN to include\n"
                                 "5. max_amount     (numeric, optional, default=25) Maximum UTXO amount in AVN to include\n"
    ...
    // Check if wallet is synced - consolidation requires accurate UTXO state
    if (IsInitialBlockDownload()) {
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Cannot consolidate UTXOs while blockchain is syncing. Wait for sync to complete.");
    }

    // Security check: Verify the destination address belongs to this wallet
    CAvianAddress address(request.params[0].get_str());
    if (!address.IsValid()) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Avian address");
    }
    ...
    if (!IsMine(*pwallet, dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
            ...
            "Use 'getnewaddress' or 'getaddressesbyaccount \"\"' to get a valid destination address.");
    }

    // Loop unlocked coins
    std::map<std::string, std::vector<COutput>> mapCoins;
    ConsolidateUtxosListCoins(mapCoins, pwallet);

    // Parse parameters using standard RPC pattern
    int utxoCount = mapCoins.size();
    int minimumUtxoAmount = 2000;
    int maxBatches = 0;                   // 0 = unlimited
    CAmount minSpendableAmount = 1000000; // 0.01 AVN default
    CAmount maxInputAmount = 2500000000;  // 25 AVN default

    if (request.params.size() > 1 && !request.params[1].isNull()) {
        minimumUtxoAmount = std::stoi(request.params[1].get_str());
    }

    if (request.params.size() > 2 && !request.params[2].isNull()) {
        maxBatches = std::stoi(request.params[2].get_str());
    }
    ...
    while (nOps > 0) {
        ...
        pwallet->AvailableCoins(vAllCoins, true, nullptr);

        // Filter and sort UTXOs by amount (smallest first) for proper dust consolidation
        std::vector<COutput> vSortedCoins;
        for (const COutput& out : vAllCoins) {
            CAmount utxoAmount = out.tx->tx->vout[out.i].nValue;

            // First priority: UTXOs in our preferred range
            if (utxoAmount >= minSpendableAmount && utxoAmount <= maxInputAmount) {
                vSortedCoins.push_back(out);
            }
        }

        // Sort by amount (smallest first) - this is key for proper dust consolidation
        ...
        // If we don't have enough small UTXOs, add some larger ones (also sorted)
        if (vSortedCoins.size() < nOdds) {
            ...
            // Sort larger coins by amount too
            std::sort(vLargerCoins.begin(), vLargerCoins.end(),
                [](const COutput& a, const COutput& b) {
                    return a.tx->tx->vout[a.i].nValue < b.tx->tx->vout[b.i].nValue;
                });

            // Add as many larger coins as needed
            int needed = std::min((int)vLargerCoins.size(), nOdds - (int)vSortedCoins.size());
            vSortedCoins.insert(vSortedCoins.end(), vLargerCoins.begin(), vLargerCoins.begin() + needed);
        }

        // Select the smallest UTXOs for consolidation with output amount limit
        ...
        // Check if any UTXOs were selected
        if (selectedCount == 0) {
            throw JSONRPCError(RPC_WALLET_ERROR, "No UTXOs available for consolidation");
        }

        // Check batch constraints
        if (selectionSum > maxOutputAmount) {
            throw JSONRPCError(RPC_WALLET_ERROR, strprintf("Selected amount %s exceeds maximum output %s",
                                                     FormatMoney(selectionSum), FormatMoney(maxOutputAmount)));
        }

        CAmount estimatedFee = (180 + (selectedCount * 150)) * 1; // 1 sat/byte minimum
        if (estimatedFee < 10000) estimatedFee = 10000;           // Minimum 0.0001 AVN fee
        if (selectionSum <= estimatedFee * 3) {
            throw JSONRPCError(RPC_WALLET_ERROR, strprintf("Selected amount %s too small for consolidation (estimated fee: %s, need at least %s)",
                                                     FormatMoney(selectionSum), FormatMoney(estimatedFee), FormatMoney(estimatedFee * 3)));
        }
        ...
        CRecipient recipient = {scriptPubKey, selectionSum, true};
        vecSend.push_back(recipient);

        obj.push_back(Pair("selected_coins", selectedCount));
        obj.push_back(Pair("selection_sum", FormatMoney(selectionSum)));
        ...
        if (!fCreated) {
            // Provide detailed error information like other wallet functions
            if (strFailReason == "Insufficient funds") {
                CAmount nBalance = pwallet->GetBalance();
                // Since we're using fSubtractFeeFromAmount=true, the output will be selectionSum - actualFee
                throw JSONRPCError(RPC_WALLET_INSUFFICIENT_FUNDS,
                    strprintf("Insufficient funds for consolidation. Selected %d UTXOs totaling %s, CreateTransaction failed with fee requirement %s (estimated: %s). Wallet balance: %s",
                        selectedCount, FormatMoney(selectionSum), FormatMoney(nFeeRequired), FormatMoney(estimatedFee), FormatMoney(nBalance)));
            } else if (nFeeRequired > 0) {
                throw JSONRPCError(RPC_WALLET_ERROR,
                    strprintf("Transaction creation failed: %s. This transaction requires a fee of at least %s",
                        strFailReason, FormatMoney(nFeeRequired)));
            } else {
                throw JSONRPCError(RPC_WALLET_ERROR,
                    strprintf("Transaction creation failed: %s", strFailReason));
            }
        }
        ...
        obj.push_back(Pair("batch_number", batchCount + 1));
        ret.push_back(obj);
    ...
    if (batchCount > 0) {
        ...
        summary.push_back(Pair("total_batches_processed", batchCount));
        summary.push_back(Pair("remaining_utxos", (int)mapCoins.size()));
        if (maxBatches > 0 && batchCount >= maxBatches) {
            summary.push_back(Pair("status", "stopped_at_batch_limit"));
        } else {
            summary.push_back(Pair("status", "completed_all_eligible_utxos"));
        }
