...
BOOST_FIXTURE_TEST_SUITE(asset_reissue_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(reissue_cache_test_ipfs)
    {
        ...
        BOOST_CHECK_MESSAGE(cache.AddNewAsset(asset1, GetBurnAddress(AssetType::ROOT), 0, hash), "Failed to add new asset");
        ...
        BOOST_CHECK_MESSAGE(cache.AddReissueAsset(reissue1, GetBurnAddress(AssetType::ROOT), out), "Failed to add reissue");
        ...
        BOOST_CHECK_MESSAGE(cache.mapAssetsAddressAmount.at(make_pair("RVNASSET", GetBurnAddress(AssetType::ROOT))) == CAmount(101 * COIN), "Reissued amount wasn't added to the previous total");
        ...
        undoBlockData.emplace_back(std::make_pair("RVNASSET", CBlockAssetUndo{true, false, false, "", "", 0, ASSET_UNDO_INCLUDES_VERIFIER_STRING, false, ""}));
        BOOST_CHECK_MESSAGE(cache.RemoveReissueAsset(reissue1, GetBurnAddress(AssetType::ROOT), out, undoBlockData), "Failed to remove reissue");
        ...
        BOOST_CHECK_MESSAGE(cache.mapAssetsAddressAmount.at(make_pair("RVNASSET", GetBurnAddress(AssetType::ROOT))) == CAmount(100 * COIN), "Assets total wasn't undone when reissuance was");
    }
    ...
    BOOST_AUTO_TEST_CASE(reissue_isvalid_test)
    {
        ...
        // Add an asset to a valid rvn address
        BOOST_CHECK_MESSAGE(cache.AddNewAsset(asset1, GetBurnAddress(AssetType::ROOT), 0, uint256()), "Failed to add new asset");
        ...
        BOOST_CHECK_MESSAGE(cache.AddNewAsset(asset2, GetBurnAddress(AssetType::ROOT), 0, uint256()), "Failed to add new asset");
        ...
        BOOST_CHECK_MESSAGE(cache.AddNewAsset(asset3, GetBurnAddress(AssetType::ROOT), 0, uint256()), "Failed to add new asset");

        // Create a reissuance of the asset that is valid txid but messaging isn't active in unit tests
        CReissueAsset reissue7("DATAHASH", CAmount(1 * COIN), -1, 1, DecodeAssetData("9c2c8e121a0139ba39bffd3ca97267bca9d4c0c1e84ac0c34a883c28e7a912ca"), "");

        BOOST_CHECK_MESSAGE(ContextualCheckReissueAsset(&cache, reissue7, error), "Reissue7 wasn't valid: " + error);
    }
