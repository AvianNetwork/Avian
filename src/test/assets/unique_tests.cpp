...
BOOST_FIXTURE_TEST_SUITE(unique_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(unique_from_transaction_test)
    {
        ...
        CScript newUniqueScript = GetScriptForDestination(DecodeDestination(GetBurnAddress(AssetType::ROOT)));
        ...
        BOOST_CHECK_MESSAGE(address == GetBurnAddress(AssetType::ROOT), "Unique Tests: Failed address check");
