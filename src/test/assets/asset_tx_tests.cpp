...
BOOST_FIXTURE_TEST_SUITE(asset_tx_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(asset_tx_valid_test)
    {
        ...
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(GetBurnAddress(AssetType::ROOT)));
        ...
        coins.AddCoin(outpoint, Coin(txOut, 10, false), true);
        ...
        BOOST_CHECK_MESSAGE(Consensus::CheckTxAssets(tx, state, coins, nullptr, nullptr, vReissueAssets, true), "CheckTxAssets Failed");
    }
    ...
    BOOST_AUTO_TEST_CASE(asset_tx_not_valid_test)
    {
        ...
        CScript scriptLess = GetScriptForDestination(DecodeDestination(GetBurnAddress(AssetType::ROOT)));
        ...
        std::vector<std::pair<std::string, uint256>> vReissueAssets;
        BOOST_CHECK_MESSAGE(!Consensus::CheckTxAssets(tx, state, coins, nullptr, nullptr, vReissueAssets, true), "CheckTxAssets should have failed");
    }
    ...
    BOOST_AUTO_TEST_CASE(asset_tx_valid_multiple_outs_test)
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            CScript scriptPubKey2 = GetScriptForDestination(DecodeDestination(GetBurnAddress(AssetType::ROOT)));
    ...
    BOOST_AUTO_TEST_CASE(asset_tx_multiple_outs_invalid_test)
    {
        ...
        std::vector<std::pair<std::string, uint256>> vReissueAssets;
        BOOST_CHECK_MESSAGE(!Consensus::CheckTxAssets(tx, state, coins, nullptr, nullptr, vReissueAssets, true), "CheckTxAssets passed when it should have failed");
    }
    ...
    BOOST_AUTO_TEST_CASE(asset_tx_multiple_assets_test)
    {
        ...
        CScript scriptPubKey3 = GetScriptForDestination(DecodeDestination(GetBurnAddress(AssetType::ROOT)));
        ...
        coins.AddCoin(outpoint2, Coin(txOut2, 10, false), true);

        COutPoint outpoint3(hash3, 1);
        coins.AddCoin(outpoint3, Coin(txOut3, 10, false), true);
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            CScript outScript = GetScriptForDestination(DecodeDestination(GetBurnAddress(AssetType::ROOT)));
            ...
            CScript outScript2 = GetScriptForDestination(DecodeDestination(GetBurnAddress(AssetType::ROOT)));
            ...
            CScript outScript3 = GetScriptForDestination(DecodeDestination(GetBurnAddress(AssetType::ROOT)));
        ...
        std::vector<std::pair<std::string, uint256>> vReissueAssets;
        BOOST_CHECK_MESSAGE(Consensus::CheckTxAssets(tx, state, coins, nullptr, nullptr, vReissueAssets, true), state.GetDebugMessage());
        ...
        BOOST_CHECK_MESSAGE(!Consensus::CheckTxAssets(tx2, state, coins, nullptr, nullptr, vReissueAssets, true), "CheckTxAssets should have failed");
    }
    ...
    // NOTE: asset_tx_enforce_value_test and asset_tx_enforce_coinbase_test
    // from v4.2 are not ported because they depend on the old CheckTransaction
    // signature (6 params) and SetEnforcedValues/SetEnforcedCoinbase which
    // don't exist in the v5.0 codebase. These tests need to be rewritten
    // to use the v5.0 validation pipeline.
