...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        Coin coin(txOut, 0, false);

        BOOST_CHECK_MESSAGE(coin.out.scriptPubKey.IsAssetScript(), "Transfer Asset Coin isn't as asset");
    }
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        BOOST_CHECK_MESSAGE(coin.out.scriptPubKey.IsAssetScript(), "New Asset Coin isn't as asset");
    }
    ...
    BOOST_AUTO_TEST_CASE(asset_invalid_check_tests)
    {
        ...
        /// Qualifier Tests ///
        {
            // Check that units must be zero for qualifiers
            CNewAsset invalid_qualifier_units("#QUALIFIER", 1 * COIN, MAX_UNIT, 0, 0, "");
            BOOST_CHECK_MESSAGE(!CheckNewAsset(invalid_qualifier_units, error), "CheckNewAsset: Invalid Qualifier Units Test should fail");

            // Check that the amount can't be bigger than 1 * COIN
            CNewAsset invalid_qualifier_amount("#QUALIFIER", 2 * COIN, MIN_UNIT, 0, 0, "");
            BOOST_CHECK_MESSAGE(!CheckNewAsset(invalid_qualifier_amount, error), "CheckNewAsset: Invalid Qualifier Amount Test should fail");

            // Check that reissue flag must be 0
            CNewAsset invalid_qualifier_reissue_flag("#QUALIFIER", 1 * COIN, MIN_UNIT, 1, 0, "");
            BOOST_CHECK_MESSAGE(!CheckNewAsset(invalid_qualifier_reissue_flag, error), "CheckNewAsset: Invalid Qualifier Reissue Flag Test should fail");
        }

        /// Sub Qualifier Tests ///
        {
            // Check that units must be zero for sub qualifiers
            CNewAsset invalid_sub_qualifier_units("#QUALIFIER/#SUB", 1 * COIN, MAX_UNIT, 0, 0, "");
            BOOST_CHECK_MESSAGE(!CheckNewAsset(invalid_sub_qualifier_units, error), "CheckNewAsset: Invalid Sub Qualifier Units Test should fail");

            // Check that the amount can't be bigger than 1 * COIN
            CNewAsset invalid_sub_qualifier_amount("#QUALIFIER/#SUB", 2 * COIN, MIN_UNIT, 0, 0, "");
            BOOST_CHECK_MESSAGE(!CheckNewAsset(invalid_sub_qualifier_amount, error), "CheckNewAsset: Invalid Sub Qualifier Amount Test should fail");

            // Check that reissue flag must be 0
            CNewAsset invalid_sub_qualifier_reissue_flag("#QUALIFIER/#SUB", 1 * COIN, MIN_UNIT, 1, 0, "");
            BOOST_CHECK_MESSAGE(!CheckNewAsset(invalid_sub_qualifier_reissue_flag, error), "CheckNewAsset: Invalid Sub Qualifier Reissue Flag Test should fail");
        }

        /// Restricted Tests ///
        {
            // Check that units must be zero for restricted assets
            CNewAsset invalid_restricted_units("$RESTRICTED", 1 * COIN, MAX_UNIT, 0, 0, "");
            BOOST_CHECK_MESSAGE(!CheckNewAsset(invalid_restricted_units, error), "CheckNewAsset: Invalid Restricted Units Test should fail");
        }
    }
