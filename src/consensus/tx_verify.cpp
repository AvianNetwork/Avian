...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, int nHeight, CAmount blockReward, bool fCheckDuplicateInputs, bool fMempoolCheck, bool fBlockCheck)
{
    ...
    for (const auto& txout : tx.vout)
    {
        ...
        /** AVN START */
        // Find and handle all new OP_AVN_ASSET null data transactions
    ...
    if (tx.IsNewAsset()) {
        ...
        // above transaction types.  Also fail if it contains OP_AVN_ASSET opcode but wasn't a valid script.
        for (auto out : tx.vout) {
            int nType;
            bool _isOwner;
            if (out.scriptPubKey.IsAssetScript(nType, _isOwner)) {
                ...
                if (out.scriptPubKey.Find(OP_AVN_ASSET)) {
                ...
                if (out.scriptPubKey.IsAssetScript(nType, _isOwner)) {
                    ...
                    if (out.scriptPubKey.Find(OP_AVN_ASSET)) {
                        if (AreRestrictedAssetsDeployed()) {
                            if (out.scriptPubKey[0] != OP_AVN_ASSET) {
                                return state.DoS(100, false, REJECT_INVALID,
                                                 "bad-txns-op-avn-asset-not-in-right-script-location", false, "", tx.GetHash());
                            }
                        } else {
