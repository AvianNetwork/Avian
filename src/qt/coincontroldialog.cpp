...
{
    GUIUtil::setClipboard(AvianUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    l2->setText(AvianUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(AvianUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(AvianUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(AvianUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
...
void CoinControlDialog::updateView()
{
    ...
    for (const std::pair<QString, std::vector<COutput>>& coins : mapCoins) {
        ...
        for (const COutput& out : coins.second) {
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, AvianUnits::format(nDisplayUnit, out.tx->tx->vout[out.i].nValue));
            itemOutput->setToolTip(COLUMN_AMOUNT, AvianUnits::format(nDisplayUnit, out.tx->tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, AvianUnits::format(nDisplayUnit, nSum));
            itemWalletAddress->setToolTip(COLUMN_AMOUNT, AvianUnits::format(nDisplayUnit, nSum));
