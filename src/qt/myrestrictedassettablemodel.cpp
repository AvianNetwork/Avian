...
{
public:
    MyRestrictedAssetsTablePriv(CWallet* _wallet, MyRestrictedAssetsTableModel* _parent) : wallet(_wallet),
                                                                                           parent(_parent)
    {
    }
...
MyRestrictedAssetsTableModel::MyRestrictedAssetsTableModel(const PlatformStyle* _platformStyle, CWallet* _wallet, WalletModel* parent) : QAbstractTableModel(parent),
                                                                                                                                         wallet(_wallet),
                                                                                                                                         walletModel(parent),
...
{
    // Update the amount column headers to show the current display unit
...
public:
    ...
    MyRestrictedTransactionNotification(std::string _address, std::string _asset_name, int _type, uint32_t _date) : address(_address), asset_name(_asset_name), type(_type), date(_date) {}
