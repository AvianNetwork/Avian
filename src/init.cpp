...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-webuibind=<addr>", strprintf("Bind address for dedicated Web UI port (default: %s)", DEFAULT_WEBUI_BIND), ArgsManager::ALLOW_ANY, OptionsCategory::WEBUI);
    argsman.AddArg("-webuiport=<port>", strprintf("Dedicated port for Web UI (default: %u; shares RPC port when unset)", DEFAULT_WEBUI_PORT), ArgsManager::ALLOW_ANY, OptionsCategory::WEBUI);
    argsman.AddArg("-webuiassets=<dir>", "Serve Web UI static files from <dir> instead of embedded assets (for development)", ArgsManager::ALLOW_ANY, OptionsCategory::WEBUI);
    argsman.AddArg("-webuipassword=<password>", "Set a fixed password for Web UI login instead of the per-session cookie token. Set once in avian.conf: webuipassword=<password>", ArgsManager::ALLOW_ANY | ArgsManager::SENSITIVE, OptionsCategory::WEBUI);
#endif
