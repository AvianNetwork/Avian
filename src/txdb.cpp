...
bool CBlockTreeDB::ReadAddressUnspentIndex(uint160 addressHash, int type, std::string assetName, std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue>>& unspentOutputs)
{
    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_ADDRESSUNSPENTINDEX && key.second.hashBytes == addressHash && (assetName.empty() || key.second.asset == assetName)) {
...
bool CBlockTreeDB::ReadAddressIndex(uint160 addressHash, int type, std::string assetName, std::vector<std::pair<CAddressIndexKey, CAmount>>& addressIndex, int start, int end)
{
    boost::scoped_ptr<CDBIterator> pcursor(NewIterator());
    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_ADDRESSINDEX && key.second.hashBytes == addressHash && (assetName.empty() || key.second.asset == assetName)) {
