...
{
    return AreAssetsDeployed() ? "" : "\nTHIS COMMAND IS NOT YET ACTIVE!\nhttps://github.com/AvianNetwork/rips/blob/master/rip-0002.mediawiki\n";
}
    ...
    if (request.params.size() > 2) {
        ...
        if (!change_address.empty()) {
            ...
            if (!IsValidDestination(change_dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Avian change address: ") + change_address);
            }
        }
    }
...
UniValue issue(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 1 || request.params.size() > 8)
        throw std::runtime_error(
            "issue \"asset_name\" qty \"( to_address )\" \"( change_address )\" ( units ) ( reissuable ) ( has_ipfs ) \"( ipfs_hash )\" ( has_ans ) \"( ans_id )\"\n" + AssetActivationWarning() +
            ...
            "9. \"has_ans\"               (boolean, optional, default=false), whether ANS data is going to be added to the asset\n"
            "10. \"ans_id\"               (string, optional but required if has_ans = 1), an ANS ID once messaging is activated\n"
        ...
        if (!change_address.empty()) {
            ...
            if (!IsValidDestination(destination)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                    std::string("Invalid Change Address: Invalid Avian address: ") + change_address);
            }
        }
    }
    ...
    if (fMessageCheck) {
        ...
        CheckIPFSTxidMessage(ans_id, expireTime);
    }
...
UniValue issueunique(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "4. \"ans_ids\"               (array, optional) ANS IDs corresponding to each supplied tag (should be same size as \"asset_tags\")\n"
            "5. \"to_address\"            (string, optional, default=\"\"), address assets will be sent to, if it is empty, address will be generated for you\n"
            "6. \"change_address\"        (string, optional, default=\"\"), address the the avn change will be sent to, if it is empty, change address will be generated for you\n"
    ...
    const UniValue& ansIDS = request.params[3];
    if (!ansIDS.isNull()) {
        if (!ansIDS.isArray() || ansIDS.size() != assetTags.size()) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("If provided, ANS IDs must be an array of the same size as the asset tags array."));
        }
    }
    ...
    if (!changeAddress.empty()) {
        ...
        if (!IsValidDestination(destination)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                std::string("Invalid Change Address: Invalid Avian address: ") + changeAddress);
        }
    }
    ...
    for (int i = 0; i < (int)assetTags.size(); i++) {
        ...
        if (ipfsHashes.isNull() && ansIDS.isNull()) {
            asset = CNewAsset(assetName, UNIQUE_ASSET_AMOUNT, UNIQUE_ASSET_UNITS, UNIQUE_ASSETS_REISSUABLE, 0, "", 0, "");
        } else if (!ipfsHashes.isNull()) {
            asset = CNewAsset(assetName, UNIQUE_ASSET_AMOUNT, UNIQUE_ASSET_UNITS, UNIQUE_ASSETS_REISSUABLE, 1,
                DecodeAssetData(ipfsHashes[i].get_str()), 0, "");
        } else { // Must be ANS
...
UniValue listassetbalancesbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"                  (string, required) a avian address\n"
...
UniValue getansdata(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("getansdata", "\"ASSET_NAME\"") + HelpExampleRpc("getansdata", "\"ASSET_NAME\""));
...
UniValue transferfromaddresses(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 4 || request.params.size() > 8)
        throw std::runtime_error(
            "transferfromaddresses \"asset_name\" [\"from_addresses\"] qty \"to_address\" \"message\" expire_time \"avn_change_address\" \"asset_change_address\"\n" + AssetActivationWarning() +
            ...
            "7. \"avn_change_address\"       (string, optional, default = \"\") the transactions AVN change will be sent to this address\n"
...
UniValue reissue(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() > 7 || request.params.size() < 3)
        throw std::runtime_error(
            ...
            "\nCan change the ANS ID during reissuance"
            ...
            "8. \"new_ans_id\"                (string, optional, default=\"\"), whether to update the current ANS ID once messaging is active\n"
    ...
    if (request.params.size() > 7) {
        fMessageCheck = true;
        newANSID = request.params[7].get_str();
    }
    ...
    if (fMessageCheck) {
        ...
        CheckIPFSTxidMessage(newANSID, expireTime);
    }

    CReissueAsset reissueAsset(asset_name, nAmount, newUnits, reissuable, DecodeAssetData(newipfs), newANSID);
...
UniValue listtagsforaddress(const JSONRPCRequest& request)
{
    ...
    CTxDestination dest = DecodeDestination(address);
    if (!IsValidDestination(dest))
        throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("Not valid AVN address: ") + address);
...
UniValue checkaddresstag(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreRestrictedAssetsDeployed() || request.params.size() != 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"          (string, required) the AVN address to search\n"
...
UniValue issuerestrictedasset(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreRestrictedAssetsDeployed() || request.params.size() < 4 || request.params.size() > 9)
        throw std::runtime_error(
            ...
            "5. \"change_address\"        (string, optional, default=\"\") address that the avn change will be sent to, if it is empty, change address will be generated for you\n"
...
UniValue reissuerestrictedasset(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreRestrictedAssetsDeployed() || request.params.size() < 3 || request.params.size() > 9)
        throw std::runtime_error(
            ...
            "6. \"change_address\"        (string, optional, default=\"\") address that the avn change will be sent to, if it is empty, change address will be generated for you\n"
...
UniValue transferqualifier(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > 3) {
        ...
        CTxDestination change_dest = DecodeDestination(change_address);
        if (!IsValidDestination(change_dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Avian address: ") + change_address);
        }
    }
...
UniValue ansencode(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2)
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("ansencode", "\"ip\" 127.0.0.1") + HelpExampleRpc("ansencode", "\"ip\" 127.0.0.1"));
    ...
    if (strType == "addr")
        ...
        throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("Invalid type: try \"addr\" or \"ip\""));
...
UniValue ansdecode(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1)
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("ansdecode", "ANS1ffffffff") + HelpExampleRpc("ansdecode", "ANS1ffffffff"));
...
//  ----------- ------------------------      -----------------------      ----------
        ...
        {"assets", "transferfromaddresses", &transferfromaddresses, {"asset_name", "from_addresses", "qty", "to_address", "message", "expire_time", "avn_change_address", "asset_change_address"}},
