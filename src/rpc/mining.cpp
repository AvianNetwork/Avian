...
static RPCHelpMan generateblock()
{
    return RPCHelpMan{"generateblock",
        ...
        {
            {"output", RPCArg::Type::STR, RPCArg::Optional::NO, "The address or descriptor to send the newly generated AVN to."},
...
// NOTE: Unlike wallet RPC (which use AVN values), mining RPCs follow GBT (BIP 22) in using avianshi amounts
static RPCHelpMan prioritisetransaction()
{
    return RPCHelpMan{"prioritisetransaction",
            ...
            "                  DEPRECATED. For forward compatibility use named arguments and omit this parameter."},
                    {"fee_delta", RPCArg::Type::NUM, RPCArg::Optional::NO, "The fee value (in avianshis) to add (or subtract, if negative).\n"
        ...
        RPCResult{
            RPCResult::Type::OBJ_DYN, "", "prioritisation keyed by txid",
            {
                {RPCResult::Type::OBJ, "<transactionid>", "", {
                    {RPCResult::Type::NUM, "fee_delta", "transaction fee delta in avianshis"},
                    {RPCResult::Type::BOOL, "in_mempool", "whether this transaction is currently in mempool"},
                    {RPCResult::Type::NUM, "modified_fee", /*optional=*/true, "modified fee in avianshis. Only returned if in_mempool=true"},
                }}
            },
        },
        RPCExamples{
    ...
    return RPCHelpMan{
        ...
        "    https://github.com/bitcoin/bips/blob/master/bip-0145.mediawiki\n",
        {
            {"template_request", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "Format of the template",
            {
                ...
                {
                    {"segwit", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "(literal) indicates client side segwit support (always active in Avian)"},
            ...
            RPCResult{"Otherwise", RPCResult::Type::OBJ, "", "",
            {
                ...
                {RPCResult::Type::ARR, "transactions", "contents of non-coinbase transactions that should be included in the next block",
                {
                    {RPCResult::Type::OBJ, "", "",
                    {
                        ...
                        {RPCResult::Type::NUM, "fee", "difference in value between transaction inputs and outputs (in avianshis); for coinbase transactions, this is a negative Number of the total collected block fees (ie, not including the block subsidy); if key is not present, fee is unknown and clients MUST NOT assume there isn't one"},
                ...
                {RPCResult::Type::NUM, "coinbasevalue", "maximum allowable input to coinbase transaction, including the generation award and transaction fees (in avianshis)"},
                ...
                {RPCResult::Type::OBJ, "founder", "founder/dev fee payment information",
                {
                    ...
                    {RPCResult::Type::NUM, "amount", "founder payment amount in avianshis"},
                }},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // AVN: SegWit is always active in Avian (from block 1). Don't require
    // callers to explicitly signal segwit support — existing miners/pools
    // may not send the "segwit" rule flag. Treat it as implicitly supported.
