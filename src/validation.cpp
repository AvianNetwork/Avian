...
bool CheckFinalTx(const CTransaction& tx, int flags)
{
    ...
    const int64_t nBlockTime = ((flags & LOCKTIME_MEDIAN_TIME_PAST) && chainActive.Tip()) ? chainActive.Tip()->GetMedianTimePast() : GetAdjustedTime();
...
static bool AcceptToMemoryPoolWorker(const CChainParams& chainparams, CTxMemPool& pool, CValidationState& state, const CTransactionRef& ptx, bool* pfMissingInputs, int64_t nAcceptTime, std::list<CTransactionRef>* plTxnReplaced, bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        if (AreAssetsDeployed()) {
            if (!Consensus::CheckTxAssets(tx, state, view, GetCurrentAssetCache(), true, vReissueAssets, false, nullptr, 0, nullptr, chainActive.Height() + 1))
        ...
        for (const CTxMemPool::txiter it : allConflicting) {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s AVN additional fees, %d delta bytes\n",
...
void UpdateCoins(const CTransaction& tx, CCoinsViewCache& inputs, CTxUndo& txundo, int nHeight, uint256 blockHash, CAssetsCache* assetCache, std::pair<std::string, CBlockAssetUndo>* undoAssetData)
{
    ...
    if (!tx.IsCoinBase()) {
        ...
        for (const CTxIn& txin : tx.vin) {
            ...
            bool is_spent = inputs.SpendCoin(txin.prevout, &txundo.vprevout.back(), assetCache); /** AVN START */ /* Pass assetCache into function */ /** AVN END */
            assert(is_spent);
        }
    }
    // add outputs
    AddCoins(inputs, tx, nHeight, blockHash, false, assetCache, undoAssetData); /** AVN START */ /* Pass assetCache into function */ /** AVN END */
}
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        for (size_t o = 0; o < tx.vout.size(); o++) {
            if (!tx.vout[o].scriptPubKey.IsUnspendable()) {
                ...
                Coin coin;
                bool is_spent = view.SpendCoin(out, &coin, &tempCache); /** AVN START */ /* Pass assetsCache into the SpendCoin function */ /** AVN END */
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                int res = ApplyTxInUndo(std::move(undo), view, out, assetsCache); /** AVN START */ /* Pass assetsCache into ApplyTxInUndo function */ /** AVN END */
...
int32_t ComputeBlockVersion(const CBlockIndex* pindexPrev, const Consensus::ConsensusParams& params)
{
    ...
    if (pindexPrev != nullptr) {
        if (IsDualAlgoEnabled(pindexPrev, params))
            nVersion = 0;
    }
...
// Dual Algo: Check if Dual Algo is activated at given point
bool IsDualAlgoEnabled(const CBlockIndex* pindexPrev, const Consensus::ConsensusParams& params)
{
    // return (VersionBitsState(pindexPrev, params, Consensus::DEPLOYMENT_DUAL_ALGO, versionbitscache) == THRESHOLD_ACTIVE);
    if (pindexPrev != nullptr) {
    ...
    try {
        {
            ...
            if (fDoFullFlush) {
                ...
                if (!CheckDiskSpace((48 * 2 * 2 * pcoinsTip->GetCacheSize()) + assetDirtyCacheSize * 2)) /** AVN START */ /** AVN END */
                    return state.Error("out of disk space");
...
void static UpdateTip(CBlockIndex* pindexNew, const CChainParams& chainParams)
{
    ...
    if (!IsInitialBlockDownload()) {
        ...
        for (int i = 0; i < 100 && pindex != nullptr; i++) {
            ...
            if (IsDualAlgoEnabled(pindex, chainParams.GetConsensus())) {
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
...

    {
        ...
        LogPrint(BCLog::BENCH, "  - Flush AVN: %.2fms [%.2fs (%.2fms/blk)]\n", (nTime4 - nTime3) * MILLI, nTimeFlush * MICRO, nTimeFlush * MILLI / nBlocksTotal);

        /** AVN START */
        nTimeAssetsFlush = GetTimeMicros();
        bool assetFlushed = assetCache.Flush();
        assert(assetFlushed);
        int64_t nTimeAssetFlushFinished = GetTimeMicros();
        nTimeAssetFlush += nTimeAssetFlushFinished - nTimeAssetsFlush;
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if (IsDGWActive(pindexPrev->nHeight + 1)) {
        ...
        int64_t max_future_block_time = IsDualAlgoEnabled(pindexPrev, consensusParams) ? MAX_FUTURE_BLOCK_TIME_DUAL_ALGO : MAX_FUTURE_BLOCK_TIME;
        if (block.GetBlockTime() > nAdjustedTime + max_future_block_time)
            return state.Invalid(false, REJECT_INVALID, "time-too-new", "block timestamp too far in the future");
    }
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::ConsensusParams& consensusParams, const CBlockIndex* pindexPrev, CAssetsCache* assetCache)
{
    ...
    int64_t nLockTimeCutoff = ((nLockTimeFlags & LOCKTIME_MEDIAN_TIME_PAST) && pindexPrev) ? pindexPrev->GetMedianTimePast() : block.GetBlockTime();
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (!ConnectBlock(block, state, &indexDummy, viewNew, chainparams, &assetCache, true)) /** AVN START */ /*Add asset to function */ /** AVN END*/
