...
You can use the `getnodeaddresses` RPC to fetch a number of onion peers known to your node; run `avian-cli help getnodeaddresses` for details.

`bitcoin rpc` can also be substituted for `avian-cli`.
...
In a typical situation, this suffices to run behind a Tor proxy:
    ...
    aviand -proxy=127.0.0.1:9050

`avian node` or `avian gui` can also be substituted for `aviand`.
...
This feature is enabled by default if Avian Core is listening (`-listen`) and
it requires a Tor connection to work. It can be explicitly disabled with
`-listenonion=0`. If it is not disabled, it can be configured using the
`-torcontrol` and `-torpassword` settings.

To see verbose Tor information in the aviand debug log, pass `-debug=tor`.
...
methods to be configured: cookie authentication or aviand's `-torpassword`
configuration option.

#### Cookie authentication

For cookie authentication, the user running aviand must have read access to
the `CookieAuthFile` specified in the Tor configuration. In some cases this is
preconfigured and the creation of an onion service is automatic. Don't forget to
use the `-debug=tor` aviand configuration option to enable Tor debug logging.

If a permissions problem is seen in the debug log, e.g. `tor: Authentication
cookie /run/tor/control.authcookie could not be opened (check permissions)`, it
can be resolved by adding both the user running Tor and the user running
aviand to the same Tor group and setting permissions appropriately.
...
your aviand's P2P listen port (8333 by default), and target addresses and ports
...
In a typical situation, where you're only reachable via Tor, this should suffice:
    ...
    aviand -proxy=127.0.0.1:9050 -externalip=7zvj7a2imdgkdbg4f2dryd5rgtrn7upivr5eeij4cicjh65pooxeshid.onion -listen

(obviously, replace the .onion address with your own). It should be noted that you still
listen on all devices and another node could establish a clearnet connection, when knowing
your address. To mitigate this, additionally bind the address of your Tor proxy:

    aviand ... -bind=127.0.0.1:8334=onion
...
for normal IPv4/IPv6 communication, use:
    ...
    aviand -onion=127.0.0.1:9050 -externalip=7zvj7a2imdgkdbg4f2dryd5rgtrn7upivr5eeij4cicjh65pooxeshid.onion -discover
