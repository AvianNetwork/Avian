...
All three Linux startup configurations assume the existence of an "avian" user
and group.  They must be created before attempting to use these scripts.
The macOS configuration assumes aviand will be set up for the current user.

Configuration
---------------------------------

Running aviand as a daemon does not require any manual configuration. You may
set the `rpcauth` setting in the `avian.conf` configuration file to override
...
All three configurations assume several paths that might need to be adjusted.
    ...
    PID file:            /var/run/aviand/aviand.pid (OpenRC and Upstart) or
                         /run/aviand/aviand.pid (systemd)
    Lock file:           /var/lock/subsys/aviand (CentOS)

The PID directory (if applicable) and data directory should both be owned by the
avian user and group. It is advised for security reasons to make the
configuration file and data directory only readable by the avian user and
group. Access to avian-cli and other aviand rpc clients can then be
...
### macOS
    ...
    Configuration file:  ~/Library/Application Support/AvianV5/avian.conf
    Data directory:      ~/Library/Application Support/AvianV5
    Lock file:           ~/Library/Application Support/AvianV5/.lock
