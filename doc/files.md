...
The data directory is the default location where the Avian Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.avianv5/`
macOS    | `$HOME/Library/Application Support/AvianV5/`
Windows  | `%LOCALAPPDATA%\AvianV5\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `avian.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Avian blocks (dumped in network format, 128 MiB per file)
...
`./`               | `avian.conf`          | User-defined [configuration settings](avian-conf.md) for `aviand` or `avian-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `aviand.pid`          | Stores the process ID (PID) of `aviand` or `avian-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `aviand` or `avian-qt`; can be specified by `-debuglogfile` option
...
`./`               | `powcache.dat`        | Proof-of-work cache; improves block validation performance
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [avian.conf](avian-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`avian-qt` uses [`QSettings`](https://doc.qt.io/qt-6/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-6/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Avian Core:
...
This table describes the files installed by Avian Core across different platforms.

| **Path**                                                   | **Description**                                                             |
|------------------------------------------------------------|-----------------------------------------------------------------------------|
| [README.md](README.md) or [readme.txt](README_windows.txt) | Project information and instructions                                        |
| avian.conf                                                 | [Generated](../contrib/devtools/gen-avian-conf.sh) configuration file       |
| bin/avian                                                  | Command-line tool for interacting with Avian. Calls other binaries below.   |
| bin/avian-cli                                              | Tool for making node and wallet RPC calls.                                  |
| bin/avian-util                                             | Miscellaneous utilities                                                     |
| bin/avian-wallet                                           | Avian wallet tool                                                           |
| bin/aviand                                                 | Avian node and wallet daemon                                                |
...
| *libexec/bench_avian*                                      | Benchmarking tool for measuring node performance                            |
| *libexec/avian-chainstate*                                 | Tool to validate and connect blocks                                         |
| libexec/avian-gui                                          | IPC-enabled alternative to `avian-qt`                                       |
| libexec/avian-node                                         | IPC-enabled alternative to `aviand`                                         |
| libexec/test_avian                                         | Unit test binary                                                            |
| *libexec/test_avian-qt*                                    | GUI-specific unit tests                                                     |
| share/man/man1/                                            | Man pages for command-line tools like `avian-cli`, `aviand`, and others     |
...
- README and avian.conf files are included in binary releases but not installed in source builds.
- On Windows, binaries have a `.exe` suffix (e.g., `avian-cli.exe`).
...
There have been multiple reports of database corruption and data loss when using this filesystem with Avian Core, see [Issue #31454](https://github.com/bitcoin/bitcoin/issues/31454) for more details.
