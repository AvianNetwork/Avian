...
- `-walletrbf` now defaults to `false`. Avian's 30-second block time means
  transactions confirm before a fee bump is practical.

#### Upgrading from legacy (non-descriptor) wallets

Bitcoin Core v30.2 uses descriptor wallets by default. Existing legacy wallets
continue to load and operate, but users are encouraged to migrate:

```
avian-cli migratewallet
```

After migration, the wallet uses output descriptors for key management, which
is required to access the full feature set of modern wallet RPCs.

### Removed RPCs

The following RPCs present in v4.2.0 have been removed as part of the Bitcoin
Core v30.2 rebase:

- **`listwatchonly`** — Use `getaddressesbylabel` or descriptor wallet imports
  (`importdescriptors`) to manage watch-only addresses.
- **`removewatchonly`** — Watch-only addresses are now part of descriptor wallet
  scopes and are managed via `importdescriptors`.
- **`liststucktransactions`** — Superseded by Core's mempool management RPCs
