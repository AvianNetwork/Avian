Avian Core version *0.15.0* is now available from:

  <https://avn.network/bin/avian-core-0.15.0/>
...
- The `estimatesmartfee` RPC interface has been changed (See [PR 10707](https://github.com/bitcoin/bitcoin/pull/10707)):
    ...
    - If Avian Core has not been running for long enough and has not seen enough blocks or transactions to produce an accurate fee estimation, an error will be returned (previously a value of -1 was used to indicate an error, which could be confused for a feerate).
- A new `estimaterawfee` RPC is added to provide raw fee data. External clients can query and use this data in their own fee estimation logic.

Multi-wallet support
--------------------

Avian Core now supports loading multiple, separate wallets (See [PR 8694](https://github.com/bitcoin/bitcoin/pull/8694), [PR 10849](https://github.com/bitcoin/bitcoin/pull/10849)). The wallets are completely separated, with individual balances, keys and received transactions.

Multi-wallet is enabled by using more than one `-wallet` argument when starting Avian, either on the command line or in the Avian config file.
...
Avian Core has supported creating opt-in replace-by-fee (RBF) transactions
...
- `uptime` returns the total runtime of the `aviand` server since its last start (See [PR 10400](https://github.com/bitcoin/bitcoin/pull/10400)).

Low-level RPC changes
---------------------

- When using Avian Core in multi-wallet mode, RPC requests for wallet methods must specify
...
- The second argument in `prioritisetransaction` has been renamed from `priority_delta` to `dummy` since Avian Core no longer has a concept of coin age priority. The `dummy` argument has no functional effect, but is retained for positional argument compatibility. See [Removal of Coin Age Priority](#removal-of-coin-age-priority).
...
- #10475 `39039b1` [RPC] getmempoolinfo mempoolminfee is a AVN/KB feerate (instagibbs)
...
- #10228 `27faa6c` build: regenerate avian-config.h as necessary (theuni)
...
- #10142 `ed09dd3` Run avian_test-qt under minimal QPA platform (ryanofsky)
...
- #10359 `329fc1d` [tests] functional tests should call AvianTestFramework start/stop node methods (jnewbery)
...
- #10556 `65cc7aa` Move stop/start functions from utils.py into AvianTestFramework (jnewbery)
...
- #10090 `8e4f7e7` Update avian.conf with example for pruning (coinables)
...
- #10651 `cef4b5c` Verify binaries from aviancore.org and avn.network (TheBlueMatt)
...
- #9999 `a328904` [LevelDB] Plug leveldb logs to avian logs (NicolasDorier)
...
- #10958 `659c096` Update to latest Avian patches for LevelDB (sipa)
