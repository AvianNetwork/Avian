...
```sh
$ aviand -signer=../HWI/hwi.py
```

`avian node` can also be substituted for `aviand`.
...
```sh
$ avian-cli createwallet "hww" true true "" true true true
```
...
```sh
$ avian-cli -rpcwallet=<wallet> getnewaddress
$ avian-cli -rpcwallet=<wallet> walletdisplayaddress <address>
```
...
```sh
$ avian-cli -rpcwallet=<wallet> sendtoaddress <address> <amount>
```
