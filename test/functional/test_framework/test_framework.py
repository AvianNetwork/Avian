...
TMPDIR_PREFIX = "avian_func_test_"
...
class Binaries:
    """Helper class to provide information about avian binaries
    ...
    def node_argv(self, **kwargs):
        "Return argv array that should be used to invoke aviand"
        return self._argv("node", self.paths.aviand, **kwargs)

    def rpc_argv(self):
        "Return argv array that should be used to invoke avian-cli"
        # Add -nonamed because "avian rpc" enables -named by default, but avian-cli doesn't
        return self._argv("rpc", self.paths.aviancli) + ["-nonamed"]

    def tx_argv(self):
        "Return argv array that should be used to invoke avian-tx"
        return self._argv("tx", self.paths.aviantx)

    def util_argv(self):
        "Return argv array that should be used to invoke avian-util"
        return self._argv("util", self.paths.avianutil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke avian-wallet"
        return self._argv("wallet", self.paths.avianwallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke avian-chainstate"
        return self._argv("chainstate", self.paths.avianchainstate)

    def _argv(self, command, bin_path, need_ipc=False):
        """Return argv array that should be used to invoke the command. It
        either uses the avian wrapper executable (if AVIAN_CMD is set or
        need_ipc is True), or the direct binary path (aviand, etc). When
        bin_dir is set (by tests calling binaries from previous releases) it
        always uses the direct path."""
        if self.bin_dir is not None:
            return [os.path.join(self.bin_dir, os.path.basename(bin_path))]
        elif self.paths.avian_cmd is not None or need_ipc:
            # If the current test needs IPC functionality, use the avian
            # wrapper binary and append -m so it calls multiprocess binaries.
            avian_cmd = self.paths.avian_cmd or [self.paths.avian_bin]
            return avian_cmd + (["-m"] if need_ipc else []) + [command]
        else:
            return [bin_path]
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    """Base class for an avian test script.

    Individual avian test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use avian-cli instead of RPC for all commands")
    ...
    def get_binary_paths(self):
        ...
        binaries = {
            ...
            "aviand": "AVIAND",
            "avian-cli": "AVIANCLI",
            "avian-util": "AVIANUTIL",
            "avian-tx": "AVIANTX",
            "avian-chainstate": "AVIANCHAINSTATE",
            "avian-wallet": "AVIANWALLET",
        }
        # Set paths to avian core binaries allowing overrides with environment
        # variables.
        ...
        # AVIAN_CMD environment variable can be specified to invoke avian
        # wrapper binary instead of other executables.
        paths.avian_cmd = shlex.split(os.getenv("AVIAN_CMD", "")) or None
        return paths
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("AvianRPC")
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the aviand
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in avian.conf
    ...
    def skip_if_no_aviand_tracepoints(self):
        """Skip the running test if aviand has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("aviand has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_aviand_zmq(self):
        """Skip the running test if aviand has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("aviand has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if avian-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("avian-wallet has not been compiled")

    def skip_if_no_avian_tx(self):
        """Skip the running test if avian-tx has not been compiled."""
        if not self.is_avian_tx_compiled():
            raise SkipTest("avian-tx has not been compiled")

    def skip_if_no_avian_util(self):
        """Skip the running test if avian-util has not been compiled."""
        if not self.is_avian_util_compiled():
            raise SkipTest("avian-util has not been compiled")

    def skip_if_no_avian_chainstate(self):
        """Skip the running test if avian-chainstate has not been compiled."""
        if not self.is_avian_chainstate_compiled():
            raise SkipTest("avian-chainstate has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if avian-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("avian-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether avian-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether avian-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_avian_tx_compiled(self):
        """Checks whether avian-tx was compiled."""
        return self.config["components"].getboolean("BUILD_BITCOIN_TX")

    def is_avian_util_compiled(self):
        """Checks whether avian-util was compiled."""
        return self.config["components"].getboolean("ENABLE_BITCOIN_UTIL")

    def is_avian_chainstate_compiled(self):
        """Checks whether avian-chainstate was compiled."""
