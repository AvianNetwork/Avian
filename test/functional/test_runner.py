...
`test/functional/test_framework/test_framework.py:AvianTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/avian_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and aviand
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    ...
    if "AVIAND" not in os.environ:
        ...
        os.environ["AVIANCLI"] = build_dir + '/src/avian-cli' + exeext
